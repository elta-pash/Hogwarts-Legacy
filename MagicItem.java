...
public abstract class MagicItem implements Tradeable, MagicEffectRealization, MagicSource {
	...
	public MagicItem(String name, int usages, int price, int weight) {
		if (name == null || name.isEmpty() || usages < 0 || price < 0 || weight < 0){
			throw new IllegalArgumentException("Invalid parameters for MagicItem");
		}
		this.name = name;
		this.usages = usages;
		this.price = price;
...
/*
	...
	public String toString(){
		return name + "(" + getOriginFormatted() + ")";
	}
	...
	public String toString() {
		...
		return String.format("[" + name + "; " + weight + " g; " + price + " " + currencyString + "; " + usages + " " + usageString() + additionalOutputString() + "]");
	}
	...
	public boolean provideMana(MagicLevel levelNeeded, int amount) {
		if (levelNeeded == null || amount < 0) {
			throw new IllegalArgumentException("levelNeeded can not be null or amount is negative;");
		}
		return true;
	}
	...
	public void takeDamagePercent(int percentage) {
		if (percentage > 100 || percentage < 0){
			throw new IllegalArgumentException("percentage is out of bonds");
		}
		double usagesDouble = usages*(1-percentage/100.0);
		usages = (int)usagesDouble;
	}
}