...
class CustomCompare implements Comparator<Potion>{
	@Override
	public int compare(Potion lop, Potion rop){
		int compName = rop.getName().compareTo(lop.getName());
		int compWeightPrice = (lop.getPrice()+ lop.getWeight()) - (rop.getPrice())+ rop.getWeight();
		return (compName == 0) ? compWeightPrice : compName;
	}
}
...
// Diese Methode muss für die Implementierung von Beispiel 1 dekommentiert werden
	...
	public static List<MagicItem> filterItems(List<MagicItem> items, int digit) {
		List<MagicItem> result = new ArrayList<MagicItem>();
...
// Methoden für Beispiel 1 Ende
	...
	public static Potion[] sortPotions(List<Potion> potions) {
		Potion[] result2 = new Potion[potions.size()];
		int i = 0;
		for (Potion res: potions) {
			result2[i] = res;
			++i;
		}
		Comparator<Potion> comp = Comparator.comparing(Potion::getName, Comparator.reverseOrder()).thenComparing(Potion->Potion.getWeight() + Potion.getPrice());
		Arrays.sort(result2, comp);
		//Arrays.sort(result2, new CustomCompare());
		return result2;
...
// Methoden für Beispiel 2 Ende
	...
	mapWizardsPerNoSpells(List<Wizard> wizards) {
		Map<Integer, Set<Wizard>> result3 = new HashMap<>();
		for (Wizard w: wizards) {
			int key_wizard = w.getNumberOfKnownSpells();
			if (!result3.containsKey(key_wizard)) {
				Set<Wizard> new_hashset = new HashSet<>();
				result3.put(key_wizard, new_hashset);
				new_hashset.add(w);
			}
			else {
				result3.get(key_wizard).add(w);
			}
		}
		//Set<Wizard> added_w = result3.computeIfAbsent(key_wizard, k -> new HashSet<>());
		//added_w.add(w);
		// or result3.computeIfAbsent(key_wizard, k-> new HashSet<>()).add(w);
		return result3;
	}
